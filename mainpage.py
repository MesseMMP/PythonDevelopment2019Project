...
class Area(Canvas):
    ...
    def buttondown(self, event):
        ...
        if matrix_grid[row][column]:
            ...
            self.create_rectangle(left_border_cell+1,
                                  top_border_cell+1,
                                  left_border_cell+self.delta_x-1,
                                  top_border_cell+self.delta_y-1,
    ...
    def create_grid(self):
        ...
        for x in range(self.start_x+self.delta_x, self.finish_x, self.delta_x):
            self.create_line(x, self.start_y+1,
                             x, self.finish_y, fill='lightgray')
        for y in range(self.start_y+self.delta_y, self.finish_y, self.delta_y):
            self.create_line(self.start_x+1, y,
                             self.finish_x, y, fill='lightgray')
            ...
            for col, cell in enumerate(row_cells):
                if cell:
                    self.create_rectangle(self.start_x+col*self.delta_x+1,
                                          self.start_y+row*self.delta_y+1,
                                          self.start_x+(col+1)*self.delta_x-1,
                                          self.start_y+(row+1)*self.delta_y-1,
                                          fill='black')
                else:
                    self.create_rectangle(self.start_x+col*self.delta_x+1,
                                          self.start_y+row*self.delta_y+1,
    ...
    def clear(self, matrix):
        ...
        for i in range(ROW_MATRIX):
            new_matrix.append([0 for j in range(COLUMN_MATRIX)])
